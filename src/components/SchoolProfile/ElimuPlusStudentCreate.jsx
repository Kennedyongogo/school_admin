...
export default function ElimuPlusStudentCreate() {
  ...
  const selectedUser = useMemo(
    () => eligibleUsers.find((u) => u.id === form.link_user_id) || null,
    [eligibleUsers, form.link_user_id]
  );
  ...
  return (
    <Box component="form" onSubmit={handleSubmit} sx={{ ...pageShellSx, minHeight: "100%" }}>
      ...
      <Stack spacing={2.5} sx={{ mt: 0.5, width: "100%" }}>
        ...
        <FormSection title="Account (user)">
          <Stack spacing={2} sx={{ width: "100%" }}>
            ...
            {selectedUser ? (
              <>
                <TextField label="Full name" fullWidth value={selectedUser.full_name || ""} slotProps={{ input: { readOnly: true } }} sx={inputSx} />
                <TextField label="Email" fullWidth value={selectedUser.email || ""} slotProps={{ input: { readOnly: true } }} sx={inputSx} />
                <TextField label="Username" fullWidth value={selectedUser.username || ""} slotProps={{ input: { readOnly: true } }} sx={inputSx} />
              </>
            ) : null}
      ...
      <Stack direction={{ xs: "column", sm: "row" }} spacing={2} justifyContent="flex-end" sx={{ mt: 3, width: "100%" }}>
